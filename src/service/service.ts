...
export class Service {
  ...
  static async vote(userVote: UserVote, resourceID: string, comment?: string) {
    const currentUser = { id: "userID" }; // Moralis.User.current();
  ...
  static async getLiderbord(id: string): Promise<Liderbord> {
    const params = { id: id };
    const liderbord: Liderbord = {
      id: "",
      topic: "",
      description: "",
      tags: [],
      resources: [],
    }; //await Moralis.Cloud.run( Service.getLiderbordByIdFunction, params );
    return liderbord;
  }

  static async searchLiderbordByName(name: string): Promise<any> {
    const params = { name: name };
    const liderbords: any = 0; // await Moralis.Cloud.run(  Service.searchLiderbordByNameFunction,  params);
    return liderbords;
  }
